...
void loop() {
  ...
  if (reached_target!=previous_state) {
    if (reached_target) Serial.print("G"); // Print G to let the user know its all good.
...
bool check_distance(){
  // Check if stepper 1 OR stepper 2 has distance left to go
  // If both distances add upto 0, then return true
